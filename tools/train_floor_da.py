# train network with domain adaptation network
...
from datasets.egocentric_wild_unsupervised import EgocentricWildDataset
...
def main():
    ...
    train_wild_dataset = EgocentricWildDataset(
        root_data_path='/HPS/Mo2Cap2Plus1/static00/ExternalEgo/External_camera_all',
        image_size=crop_size)
    train_wild_loader = torch.utils.data.DataLoader(
        train_wild_dataset,
        batch_size=config.TRAIN.BATCH_SIZE_PER_GPU * len(gpus),
        shuffle=config.TRAIN.SHUFFLE,
        num_workers=config.WORKERS,
    ...
    if config.TRAIN.OPTIMIZER == 'sgd':
        ...
        discriminator_optimizer = torch.optim.Adam(params=model.parameters(), lr=config.TRAIN.LR,
                                                   weight_decay=config.TRAIN.WD)
